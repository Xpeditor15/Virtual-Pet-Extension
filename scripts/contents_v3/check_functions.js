...
function reachedEdge() { //checks if the image is still in viewport after pet movement
...
function checkPosition(event) { //checks if the image is still in viewport after petDrag
    const maxLeft = 20;
    const maxRight = viewport.viewportWidth - 40;
    const maxTop = 20;
    const maxBottom = viewport.viewportHeight - 20;
